...
let validate_program (globalvars, funcs) =
    ...
    in let update_env_map curr_env functions =
        ...
        in List.fold_left (fun env argument -> 
    ...
    in let rec process_func_body fname env = function
        ...
        | Declare(decl) -> 
            let _ = if (print_debug) then (
                ...
                string_of_data_type decl ^ "\n"))
            in add_local_var_to_map (id_of_data_type fname) decl env
        | DeclareAssign(decl, exp) -> 
            let _ = if (print_debug) then (
                print_string (id_of_data_type fname ^ " Local Variable " ^
                string_of_data_type decl ^ "\n"))
            in let new_env = add_local_var_to_map (id_of_data_type fname) decl env
        ...
        | Pfor(threads, init, cond, inc, body) ->
