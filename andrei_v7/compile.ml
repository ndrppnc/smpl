...
let build_pfor (f,n,i,l,c,num_indent,env) =
	...
	indent num_indent ^ "pfor_args_" ^ k ^ "[2] = pfor_local_vars_" ^ k ^ ";\n" ^	
...
let rec convert_stmt num_indent env currf is_pfor is_replace myA myB = function
  ...
  | Pfor(t, k, i, b, s) -> let build_pfor_func (stmt,n,env) =
				...
				let counter = convert_expr env is_pfor is_replace myA myB k in
        			(indent 1) ^ "int " ^ counter ^ ";\n" ^
				(indent 1) ^ "pthread_mutex_t pfor_global_lock=PTHREAD_MUTEX_INITIALIZER;\n" ^
        			(*(indent 1) ^"int *p = (int *)(*args;\n" ^*)
        			(indent 1) ^ "int lower = (int *)args[0];\n" ^
        			(indent 1) ^ "int upper = (int *)args[1];\n" ^
        			(fst (List.fold_left (fun (acc, count) (var_id, var_type) ->
            				(acc ^ indent num_indent ^ (convert_data_type var_type) ^
					 " " ^ var_id ^ " = " ^ "(int)args[2][" ^ string_of_int count ^ "];\n", count+1)
        			) ("",0) (List.rev (get_func_locals_list currf env)))) ^
			...
			let globals_in_assignments = List.fold_left (fun acc (var_id,var_type) ->
									acc ^ "    " ^ convert_data_type var_type ^ " temp_" ^ var_id ^ " = " ^ var_id ^ ";\n"
