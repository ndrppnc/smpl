...
let build_pfor (n,l,i) =
	...
	"\tpfor_uppers_" ^ k ^ "[pfor_i_" ^ k ^ "] = (pfor_limit_" ^ k ^ "/num_threads_" ^ k ^ ")*(pfor_i_" ^ k ^ "+1)+(pfor_limit_"^ k ^"%num_threads_" ^ k ^ ");\n" ^
	"\tpfor_lowers_" ^ k ^ "[pfor_i_" ^ k ^ "] = (pfor_limit_" ^ k ^ "/num_threads_" ^ k ^ ")*pfor_i_" ^ k ^ ";\n" ^
  ...
  | Spawn(Call(id,args)) -> myenv.(1) <- myenv.(1)+1;
			...
			(build_args (args,thread)) ^ "pthread_create(&threads[" ^ thread ^ "],NULL,thread_" ^ string_of_int (myenv.(1)-1) ^ ",(void *)"
						   ^ (if List.length args > 0 then
							...
							"NULL")
						   ^ ");\n" 
  | Pfor(t, i, c, u, s) -> myenv.(2) <- myenv.(2)+1;
			   build_pfor (int_of_string (convert_expr t),1000,myenv.(2)-1)
